...
struct sockaddr_in *thread_client[COUNT_PTHREAD];
int thread_busy[COUNT_PTHREAD] = {0};
...
void *thread_server(void *arg){
    ...
    while(1){
        while(1){
            // Проверяем свободен ли поток
            if(thread_busy[*id]) break;
            usleep(1000);
        }
        pthread_mutex_lock(&mutex);
        struct sockaddr_in *client = thread_client[*id];
        ...
        // Уведомляем сервер, что поток свободен
        thread_busy[*id] = 0;
        close(fd_server_thread_socket);
        free(client);
    }
...
int main(void){
    ...
    while(1){
        ...
        // Разделяем нагрузку между потоками и проверяем их доступность
        for (int i = round_robin_counter % 3; i < COUNT_PTHREAD; i++){
            if(!thread_busy[i]){
                thread_busy[i] = 1;
                thread_client[i] = malloc(sizeof(struct sockaddr_in));
                *thread_client[i] = client;
                round_robin_counter++;
                break;
            }
        }
